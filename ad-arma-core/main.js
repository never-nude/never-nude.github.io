...
function unitColors(side) {
  ...
  function qualityStroke(q) {
    switch (q) {
      case 'veteran': return '#f2c84e';
      case 'regular': return '#f1f3f7';
      default: return '#4de978';
    }
  }

  function drawQualityFramedRing(cx, cy, quality) {
    const ringR = R * 0.55;
    const ringW = Math.max(3, Math.floor(R * 0.16));
    const frameW = Math.max(1.5, Math.floor(ringW * 0.36));
    const frameOffset = (ringW * 0.5) - (frameW * 0.5);
    const frameColor = 'rgba(8, 10, 14, 0.95)';

    // Outer dark frame so rings stay readable on light terrain.
    ctx.beginPath();
    ...
    // Dark frame on the inside of the quality band.
    ctx.beginPath();
    ctx.arc(cx, cy, ringR - frameOffset, 0, Math.PI * 2);
    ctx.lineWidth = frameW;
    ctx.strokeStyle = frameColor;
