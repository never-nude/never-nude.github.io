...
// F1: environment food spawner (keeps the world alive; click-spawn remains for debugging)
const AUTO_FOOD = true;
const AUTO_FOOD_INTERVAL = 1.8; // seconds (fast enough to be obvious)
const AUTO_FOOD_CAP = 22;       // max food objects in world
const AUTO_FOOD_MIN_R = 3.0;    // spawn ring around player
const AUTO_FOOD_MAX_R = 9.0;
let autoFoodTimer = 0;
...
function spawnAmbientFood() {
  // Spawn around the player in a ring so it usually appears on screen soon.
  const ang = Math.random() * Math.PI * 2;
  const r = rand(AUTO_FOOD_MIN_R, AUTO_FOOD_MAX_R);
...
function update(dt) {
  ...
  // F1: passive environment spawner (paused during mutation draft)
  if (!draftOpen && AUTO_FOOD) {
    autoFoodTimer += dt;
    if (autoFoodTimer >= AUTO_FOOD_INTERVAL) {
      autoFoodTimer = 0;
      if (foods.length < AUTO_FOOD_CAP) spawnAmbientFood();
    }
  }
  ...
  ui.status.textContent =
    ...
    `stage=${stageName(maturity)} | energy=${creature.energy.toFixed(1)} | foods=${foods.length}/${AUTO_FOOD_CAP} | mate=${mateStatus} | ${state}`;
}
