...
const EAT_R_PREY  = 1.15; // slightly easier to bite moving prey
const EAT_SHOW_R  = 2.40; // show target ring when near

let eatHint = "none"; // "flora" | "fauna" | "none"
let eatTarget = null; // { type, i, mesh, value, d }
...
function eatRadiusFor(type) {
  return type === "fauna" ? EAT_R_PREY : EAT_R_PLANT;
}
function ringColorFor(type) {
  return type === "fauna" ? 0xff6677 : 0xa0ffaa;
}
...
function updateEatTarget(dt) {
  ...
  // faint when near; bright + pulsing when bite-ready
  targetRing.material.opacity = ready ? 0.95 : 0.32;
  const pulse = ready ? (0.85 + 0.15 * Math.sin(worldT * 10.0)) : 1.0;
...
let worldT = 0; // world time for animation pulses/zigzags

// ----- Prey movement -----
const PREY_MAX = 2.05;       // slightly easier to catch early
const PREY_DRAG = 2.8;
const PREY_WANDER = 0.65;
const PREY_FEAR_R = 5.2;
const PREY_FEAR_FORCE = 11.5;
const PREY_ZIG = 3.4;        // readable dodge when fleeing
...
function updatePrey(dt) {
  for (let i = 0; i < prey.length; i++) {
    ...
    // flee player (with zigzag so it "feels alive" and readable)
    const dx = f.mesh.position.x - creature.pos.x;
    const dz = f.mesh.position.z - creature.pos.z;
    const d2 = dx*dx + dz*dz;
    ...
    if (d2 < PREY_FEAR_R * PREY_FEAR_R) {
      ...
      const px = -(dz / d);
      const pz = (dx / d);
      const zig = Math.sin(worldT * 12.0 + i * 1.7) * (PREY_ZIG * t);
      ax += px * zig;
      az += pz * zig;
    }

    // keep fauna near the action
