...
// === APEX_RENDER_SMOOTH ===
// Smooth only BIG position jumps (jerk reduction) + clamp NaN/black colors.
try {
  ...
  scene.traverse((o) => {
    ...
    // 1) Color clamp: if something goes NaN or full-black, push to neutral grey.
    const m = o.material;
    const c = m.color;
    ...
    // 2) Big-jump smoothing: keep "smoothPos" memory; only smooth when jump is large.
    ...
    tmp.copy(o.position); // target (already set by sim this frame)
    ...
    if (dist > 0.55) {
      ...
      sp.copy(tmp); // no smoothing for small motions (keeps player responsive)
    }
  });
} catch (e) {}
