...
// CAM_BEHIND (BUILD0007): third-person behind-follow that pivots with organism heading
let camYaw = 0.0;      // radians (Q/E)
let camDist = 10.5;    // units (Z/V)
let camHeight = 6.5;   // units
const CAM_SMOOTH = 10.0;

const CAM_Y_AXIS = new THREE.Vector3(0, 1, 0);
...
function update(dt) {
  ...
  // Camera: third-person behind-follow (pivots with heading). Q/E rotate, Z/V zoom.
  const turnSpeed = 1.8;
  if (keys.has("q")) camYaw += turnSpeed * dt;
  if (keys.has("e")) camYaw -= turnSpeed * dt;

  const zoomSpeed = 10.0;
  if (keys.has("z")) camDist = clamp(camDist - zoomSpeed * dt, 6.0, 22.0);
  if (keys.has("v")) camDist = clamp(camDist + zoomSpeed * dt, 6.0, 22.0);

  const yaw = organism.rotation.y + camYaw;

  _camTmp.set(-camDist, camHeight, 0);
  _camTmp.applyAxisAngle(CAM_Y_AXIS, yaw);

  _camDesired.copy(creature.pos).add(_camTmp);
  _camDesired.y = Math.max(1.0, _camDesired.y);

  const a = 1 - Math.exp(-dt * CAM_SMOOTH);
  camera.position.lerp(_camDesired, a);

  _lookDesired.copy(creature.pos);
  _lookDesired.y += 0.55;
  _camLook.lerp(_lookDesired, a);
