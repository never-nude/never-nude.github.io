...
const BUILD = "BUILD0043_DANGERBITE1";
const PROJECT = "Apex";
...
// ----- Courtship signal (BUILD0043_DANGERBITE1) -----
...
// ----- Mate choice system (two candidates + glyphs; BUILD0043_DANGERBITE1) -----
const mates = [];
let mateStatus = "none";
let mateSummary = "";
let selectedMate = null;

// If BUILD0043_DANGERBITE1 signal exists, use it; else fall back.
...
// === APEX_RENDER_SMOOTH ===
try {
  if (!window.__apexTmpV) window.__apexTmpV = new THREE.Vector3();
  const tmp = window.__apexTmpV;

  scene.traverse((o) => {
    if (!o || !o.isMesh || !o.position || !o.material) return;

    // Color clamp (prevents black/NaN)
    const m = o.material;
    const c = m.color;
    if (c && (!Number.isFinite(c.r) || !Number.isFinite(c.g) || !Number.isFinite(c.b) || (c.r + c.g + c.b) < 0.02)) {
      c.setRGB(0.85, 0.85, 0.85);
    }

    // Big-jump smoothing
    if (!o.userData) o.userData = {};
    if (!o.userData.__apexSmoothPos) {
      o.userData.__apexSmoothPos = o.position.clone();
      return;
    }
    const sp = o.userData.__apexSmoothPos;
    tmp.copy(o.position);

    const dist = Math.hypot(tmp.x - sp.x, tmp.y - sp.y, tmp.z - sp.z);
    if (dist > 0.55) {
      sp.lerp(tmp, 0.25);
      o.position.copy(sp);
    } else {
...
// ----- Lineage slots (BUILD0043_DANGERBITE1) -----
...
// ----- Herd Boon (BUILD0043_DANGERBITE1) -----
...
// ----- Stragglers + Sheep Eat + Eat FX (BUILD0043_DANGERBITE1) -----
...
// ----- Predators (BUILD0043_DANGERBITE1) -----
...
// ----- Prey Tiers (BUILD0043_DANGERBITE1) -----
...
// ----- Apex HUD toggle + ring purge (BUILD0043_DANGERBITE1) -----
...
// ----- Predators hunt sheep (BUILD0043_DANGERBITE1) -----
...
// ----- Metabolic Scaling + Trophic Shift (BUILD0043_DANGERBITE1) -----
...
// ----- Simple Biome + Bright Sky (BUILD0043_DANGERBITE1) -----
...
// ----- PRESSURE MINIHUD (BUILD0043_DANGERBITE1) -----
...
// DARK MODE CLAMP (BUILD0043_DANGERBITE1)
...
// ----- EVO DRAFT KERNEL (BUILD0043_DANGERBITE1) -----
...
(() => {
  if (window.__apex_evodraft_installed) return;
  window.__apex_evodraft_installed = true;

  const statusEl = document.getElementById("status");
  if (!statusEl) return;

  const TRAIT_KEY = "apex_traits_v1";
  const traits = {
    plantMult: 1.00,   // scales plant-like foods
    meatMult:  1.00,   // scales meat-like foods
    metabMult: 1.00    // scales the trophic extra drain (1=baseline)
  };

  // load
  try {
    const raw = localStorage.getItem(TRAIT_KEY);
    if (raw) {
      const obj = JSON.parse(raw);
      if (obj && typeof obj === "object") {
        if (typeof obj.plantMult === "number") traits.plantMult = obj.plantMult;
        if (typeof obj.meatMult  === "number") traits.meatMult  = obj.meatMult;
        if (typeof obj.metabMult === "number") traits.metabMult = obj.metabMult;
      }
    }
  } catch (_) {}

  window.__apex_traits = traits;

  function save() {
    try { localStorage.setItem(TRAIT_KEY, JSON.stringify(traits)); } catch (_) {}
  }
  ...
  function overlaysOpen() {
    ...
    const ov = document.getElementById("overlay");
    if (ov && !ov.classList.contains("hidden")) return true;
    const evo = document.getElementById("apexEvoOverlay");
    if (evo && !evo.classList.contains("hidden")) return true;
    return false;
  }

  function setToast(msg) {
    try {
      if (typeof window.__emerge_setEatStatus === "function") window.__emerge_setEatStatus(msg);
      else statusEl.setAttribute("data-eat", msg);
    } catch (_) {}
  }
  ...
  function getPress() {
    try {
      const a = parseFloat(statusEl.getAttribute("data-press"));
  ...
  // -------- Diet tracking (data-derived) --------
  const diet = {
    plant: 0, // plant/grazer/p0
    meat:  0, // p1/p2/meat/sheep
    total: 0
  };

  function classify(msg) {
    const m = msg.toLowerCase();
    if (m.includes("p0") || m.includes("plant") || m.includes("grazer")) return "plant";
    if (m.includes("p1") || m.includes("p2") || m.includes("meat") || m.includes("sheep") || m.includes("prey")) return "meat";
    return "other";
  }

  // -------- Trait application on nutrition (after trophic) --------
  let lastEat = "";
  const obs = new MutationObserver(() => {
    const msg = statusEl.getAttribute("data-eat") || "";
    if (!msg) return;
    if (msg === lastEat) return;
    lastEat = msg;

    const arrowCount = (msg.match(/→/g) || []).length;
    if (arrowCount < 1) return;      // wait for trophic block to annotate (+base→+trophic)
    if (arrowCount >= 2) return;     // already trait-applied

    // parse +numbers in the string
    const nums = [];
    const re = /\+(\d+)/g;
    let mm;
    while ((mm = re.exec(msg)) !== null) {
    ...
    if (kind === "plant") { diet.plant++; diet.total++; }
    else if (kind === "meat") { diet.meat++; diet.total++; }

    let mult = 1.0;
    if (kind === "plant") mult = traits.plantMult;
    if (kind === "meat")  mult = traits.meatMult;

    const final = Math.max(1, Math.round(cur * mult));
    const delta = final - cur;

    const e = energyRef();
    if (e && delta !== 0) {
      const v = e.get();
      if (typeof v === "number" && Number.isFinite(v)) e.set(Math.max(0, v + delta));
    }

    // annotate: (+base→+cur→+final)
    try {
      const patched = msg.replace(/\)\s*$/, `→+${final})`);
      statusEl.setAttribute("data-eat", patched);
    } catch (_) {}
  });
  obs.observe(statusEl, { attributes:true, attributeFilter:["data-eat"] });

  // -------- Metabolic scaling correction (trait-controlled) --------
  // This multiplies the trophic extra-drain by metabMult without editing the original code.
  ...
  function ensureUI() {
    ...
    overlay.innerHTML = `
      <div id="apexEvoPanel">
        <div id="apexEvoRow">
          <div class="apexEvoCard" data-i="0">
            <div class="apexEvoTop"><div class="apexEvoIcon">🌿</div><div class="apexEvoKey">1</div></div>
            <div class="apexEvoBars">
              <div class="apexBarRow"><div class="apexBarSym">🌿</div><div class="apexBarTrack"><div class="apexBarFill pos" data-bar="p"></div></div></div>
              <div class="apexBarRow"><div class="apexBarSym">🦷</div><div class="apexBarTrack"><div class="apexBarFill neg" data-bar="m"></div></div></div>
              <div class="apexBarRow"><div class="apexBarSym">🫀</div><div class="apexBarTrack"><div class="apexBarFill neg" data-bar="d"></div></div></div>
            </div>
          </div>

          <div class="apexEvoCard" data-i="1">
            <div class="apexEvoTop"><div class="apexEvoIcon">🦷</div><div class="apexEvoKey">2</div></div>
            <div class="apexEvoBars">
              <div class="apexBarRow"><div class="apexBarSym">🌿</div><div class="apexBarTrack"><div class="apexBarFill neg" data-bar="p"></div></div></div>
              <div class="apexBarRow"><div class="apexBarSym">🦷</div><div class="apexBarTrack"><div class="apexBarFill pos" data-bar="m"></div></div></div>
              <div class="apexBarRow"><div class="apexBarSym">🫀</div><div class="apexBarTrack"><div class="apexBarFill neg" data-bar="d"></div></div></div>
            </div>
          </div>

          <div class="apexEvoCard" data-i="2">
            <div class="apexEvoTop"><div class="apexEvoIcon">🫀</div><div class="apexEvoKey">3</div></div>
            <div class="apexEvoBars">
              <div class="apexBarRow"><div class="apexBarSym">🌿</div><div class="apexBarTrack"><div class="apexBarFill neg" data-bar="p"></div></div></div>
              <div class="apexBarRow"><div class="apexBarSym">🦷</div><div class="apexBarTrack"><div class="apexBarFill neg" data-bar="m"></div></div></div>
              <div class="apexBarRow"><div class="apexBarSym">🫀</div><div class="apexBarTrack"><div class="apexBarFill pos" data-bar="d"></div></div></div>
            </div>
          </div>
        </div>

        <div id="apexEvoHint">
          <div>U open • 1/2/3 pick</div>
          <div>Esc close</div>
        </div>
      </div>
    `;
    overlay.addEventListener("click", (e) => {
      if (e.target === overlay) closeDraft();
    });
  ...
  function makeOffers() {
    ...
    return [
      ...
      { kind:"meat",  pm: 0.95,    mm: meatUp, dm: costUp },
      { kind:"eff",   pm: 0.99,    mm: 0.99,   dm: effDown }
    ];
  }
  ...
  function renderOffers() {
    ...
    for (const card of overlay.querySelectorAll(".apexEvoCard")) {
      ...
      if (bd) {
        ...
        bd.classList.toggle("neg", o.dm > 1.0);
      }
    }
  }

  function openDraft(reason="") {
    ensureUI();
    offers = makeOffers();
    renderOffers();
    const overlay = document.getElementById("apexEvoOverlay");
  ...
  function applyOffer(i) {
    ...
    traits.meatMult  *= o.mm;
    traits.metabMult *= o.dm;

    // keep it sane
    traits.plantMult = Math.max(0.55, Math.min(2.40, traits.plantMult));
    traits.meatMult  = Math.max(0.55, Math.min(2.40, traits.meatMult));
    traits.metabMult = Math.max(0.55, Math.min(1.85, traits.metabMult));

    save();
    closeDraft();

    // non-wordy confirmation
    setToast("🧬✓");
  }
...
(() => {
    ...
    const mo = new MutationObserver((mutList) => {
      for (const m of mutList) {
        if (m.type === 'attributes' && m.attributeName === 'data-eat') {
          const v = statusEl.getAttribute('data-eat') || '';
  ...
  window.addEventListener('keydown', (e) => {
    ...
    const before = statusEl ? (statusEl.getAttribute('data-eat') || '') : '';
    setTimeout(() => {
      const after = statusEl ? (statusEl.getAttribute('data-eat') || '') : '';
      if (after && after !== before) return; // engine handled it
      ...
      const after2 = statusEl ? (statusEl.getAttribute('data-eat') || '') : '';
      if (statusEl && (!after2 || after2 === after)) {
...
// === APEX PATCH: ECOCHAIN1 (grazers consume flora; predators consume grazers) ===
(() => {
  const statusEl = document.getElementById('status');
  const toast = document.getElementById('apexToast'); // created by LEGIBILITY1
  let toastT = 0;
  let lastToastAt = 0;
  ...
  function toastMsg(msg) {
    ...
    if (now - lastToastAt < 850) return; // rate limit
    lastToastAt = now;
  ...
  // direct eval (module-scope) lets us check for variables by name safely
  function getVar(name) {
    try { return eval(name); } catch (_) { return undefined; }
  }

  function pickArray(names) {
    for (const n of names) {
      const v = getVar(n);
      if (Array.isArray(v)) return { name: n, arr: v };
    }
    return { name: null, arr: null };
  }

  // Try to find the Three.js scene
  const SCENE = (() => {
    for (const n of ['scene','world','worldScene','sc']) {
      const v = getVar(n);
      if (v && (v.isScene || v.type === 'Scene')) return v;
    }
    return null;
  })();

  // Heuristic array picks (covers most prototypes)
  const FLORA = pickArray(['flora','plants','pfl','plant','plantItems','foodsFlora','floraItems']);
  const GRAZERS = pickArray(['grazers','grazer','grazerAgents','herbivores','grazerItems','agentsGrazers']);
  const PREDS = pickArray(['preds','predators','predatorAgents','hunters','predItems','agentsPreds']);

  function obj3dOf(x) {
    if (!x) return null;
    if (x.isObject3D) return x;
    if (x.o && x.o.isObject3D) return x.o;
    if (x.mesh && x.mesh.isObject3D) return x.mesh;
    if (x.obj && x.obj.isObject3D) return x.obj;
    if (x.m && x.m.isObject3D) return x.m;
    return null;
  }
  ...
  function removeFromWorld(arr, idx) {
    const ent = arr[idx];
    arr.splice(idx, 1);
    const o = obj3dOf(ent);
    if (o && SCENE) {
  ...
  function nearestIndex(srcPos, arr) {
    let best = -1;
    let bestD = Infinity;
    for (let i = 0; i < arr.length; i++) {
      const p = posOf(arr[i]);
      if (!p) continue;
      const d = srcPos.distanceTo(p);
      if (d < bestD) { bestD = d; best = i; }
    }
  ...
  function tickEcoChain() {
    if (!FLORA.arr || !GRAZERS.arr) return;

    // Grazers eat flora if close enough
    for (let g = 0; g < GRAZERS.arr.length; g++) {
      const gp = posOf(GRAZERS.arr[g]);
      if (!gp) continue;

      const { idx, d } = nearestIndex(gp, FLORA.arr);
      if (idx !== -1 && d < 0.85) {
        removeFromWorld(FLORA.arr, idx);
        grazeEats++;
        if (statusEl) statusEl.setAttribute('data-eco', `GRAZE+1 total=${grazeEats}`);
        toastMsg('GRAZER ate flora');
      }
    }

    // Predators eat grazers if close enough
    if (PREDS.arr && PREDS.arr.length && GRAZERS.arr && GRAZERS.arr.length) {
      for (let p = 0; p < PREDS.arr.length; p++) {
        const pp = posOf(PREDS.arr[p]);
        if (!pp) continue;

        const { idx, d } = nearestIndex(pp, GRAZERS.arr);
        if (idx !== -1 && d < 0.95) {
          removeFromWorld(GRAZERS.arr, idx);
          predEats++;
          if (statusEl) statusEl.setAttribute('data-eco', `PRED+1 total=${predEats}`);
          toastMsg('PREDATOR got a grazer');
        }
      }
    }
  }

  setInterval(tickEcoChain, 350);

  console.log('[APEX ECOCHAIN1]', {
    sceneFound: !!SCENE,
    floraArray: FLORA.name,
    grazersArray: GRAZERS.name,
    predsArray: PREDS.name
  });
})();
...
/* APEX_DANGERLITE1 */
(() => {
  const LOG = (...a) => console.log("[APEX DANGERLITE1]", ...a);

  const scene = (typeof SCENE !== "undefined") ? SCENE
              : (typeof scene !== "undefined") ? scene
              : null;

  const groupChildren = (g) => {
    if (!g) return [];
  ...
  const player = findPlayerMesh();

  // ----- (1) Ring cleanup: hide ring meshes stuck at origin -----
  const hideOriginRings = () => {
    if (!scene || typeof scene.traverse !== "function") return;
    scene.traverse((o) => {
      if (!o || !o.isMesh || !o.visible) return;
      const gt = o.geometry && o.geometry.type ? o.geometry.type : "";
      const nm = (o.name || "").toLowerCase();
      const isRing = (gt === "RingGeometry") || nm.includes("ring");
      
ring.visible = false; // APEX: ring default hidden
if (!isRing) return;

      const len = Math.hypot(o.position.x, o.position.y, o.position.z);
  ...
  const SHEEP_G  = (typeof SHEEP !== "undefined") ? SHEEP : ((typeof HERD !== "undefined") ? HERD : null);
  const GRAZERS_G = (typeof GRAZERS !== "undefined") ? GRAZERS : null;
  const PREDS_G   = (typeof PREDS !== "undefined") ? PREDS : null;

  const clampTick = () => {
    // Sheep calm unless player near any sheep (panic)
    let sheepStep = 0.12;
    if (player && SHEEP_G) {
      for (const s of groupChildren(SHEEP_G)) {
        if (!s || !s.position) continue;
        const d = s.position.distanceTo(player.position);
        if (d < 6.0) { sheepStep = 0.26; break; }
      }
    }
    for (const s of groupChildren(SHEEP_G)) clampStep(s, sheepStep);
    for (const g of groupChildren(GRAZERS_G)) clampStep(g, 0.22);
    for (const p of groupChildren(PREDS_G)) clampStep(p, 0.26);
  };
  ...
  const eatClosest = (group, radius, gain, label) => {
  ...
  const _eatClosest = (group, radius, gain, label) => {
    if (!player || !group) return false;
    const items = groupChildren(group);
    if (!items.length) return false;
  ...
  window.addEventListener("keydown", (e) => {
    ...
    const ate = _eatClosest(PREDS_G,   1.10, 28, "PRED")
  ...
  const threatTick = () => {
    ...
    let hit = false;
    for (const p of groupChildren(PREDS_G)) {
...
function apexEnsureDangerRing(scene){
  ...
  const geo = new THREE.RingGeometry(__APEX_DANGER.ringIn, __APEX_DANGER.ringOut, 48);
  const mat = new THREE.MeshBasicMaterial({
    color: 0xffa500,
    transparent: true,
    opacity: 0.45,
