(function(){
  ...
  const state = {
    meta: { build, port, feature: "UNITS_EDITOR_V0_TRUTH_TOGGLE" },
  ...
  function renderBoard(){
    ...
    $("#boardMeta").textContent =
      `board: rows=${ROWS} rowCounts=[${ROW_COUNTS.join(", ")}] total=${state.board.cells.length} (expected 157) midX=${state.board.midX.toFixed(2)} units=${state.units.length} selected=${selectedText}`;
  }
  ...
  function init(){
    $("#build").textContent = `Ad Arma v2 — BUILD ${build} — ${window.location.origin}`;
