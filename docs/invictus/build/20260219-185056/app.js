...
function setupGame(layout, scenario) {
  ...
  function computeMoveSetForSelected() {
    ...
    if (!u) return;
        ...
        // MOVE_ONCE_PER_TURN_V1_MOVESET
    if (u.lastMovedTick === turnTick) return;
  ...
  function tryMoveSelectedTo(r, c) {
    ...
    if (unitAt(r, c)) {
      ...
      return;
        ...
        // MOVE_ONCE_PER_TURN_V1_GUARD
    if (u.lastMovedTick === turnTick) {
      setStatus("This unit already moved this turn.");
      return;
    }
const from = `(${u.r},${u.c})`;
    u.r = r; u.c = c;
    activationsUsed += 1;
        // MOVE_ONCE_PER_TURN_V1_MARK
    u.lastMovedTick = turnTick;
