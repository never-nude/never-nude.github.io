...
function unitColors(side) {
  ...
  function renderCombatBreakdown(rolls, info) {
    ...
    elCombatTerrain.textContent =
      `Defense modifiers: ${terrainMath}. ${braceRuleText}`;
  ...
  function retreatPick(attackerKey, defenderKey) {
    ...
    if (bh.terrain === 'water' || bh.terrain === 'mountains') return null;
  ...
  function applyImportedState(raw, sourceLabel = 'import', options = {}) {
    ...
    state.victoryMode = VICTORY_MODE_IDS.has(payload.victoryMode) ? payload.victoryMode : 'clear';
  ...
  function selectUnit(hexKey) {
    ...
    if (disarray) {
      notes.push('Disarrayed: this activation can only recover/pass.');
    }
